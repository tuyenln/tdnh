...
function wpdev_before_after($content='') {
	if (is_single()) { // is_page() ||
		$url = get_field('post_banner');
		$beforecontent = '';
		if ( $url != false ) {
			$beforecontent = "<img src='" . $url . "' class='product_banner_image' />";
		}
...
class WCB_Category_Banner {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_before_main_content', array( $this, 'wcb_show_category_banner'), 18, 0 );

		add_action( 'woocommerce_before_main_content', array( $this, 'wcb_show_product_banner'), 18, 0 );

		add_action( 'woocommerce_before_main_content', array( $this, 'wcb_show_product_banner'), 18, 0 );

		// add_action( 'before_content_container', array( $this, 'wcb_show_post_banner'), 18, 0 );
	...
	*/
	...
	public function wcb_show_product_banner() {
		// Make sure this is a product product page
		if ( is_product() ) {
			$url = get_field('product_banner');
			if ( $url != false ) {
				echo "<img src='" . $url . "' class='product_banner_image' />";
			}
			the_title( '<div class="col-xs-12"><div class="m-header"><div><h3><span>', '</span></h3></div></div></div>' );
		}
	}



	public function wcb_show_post_banner() {
		echo 123;die;
		// Make sure this is a product product page
		if ( is_single() ) {
			$url = get_field('post_banner');
